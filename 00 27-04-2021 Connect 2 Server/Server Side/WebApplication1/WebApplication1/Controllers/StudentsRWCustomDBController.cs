...
namespace WebApplication1.Controllers
{
    ...
    public class StudentsRWCustomDBController : ApiController
    {
        ...
        //api/studentsRW/2
        [Route("{id:int:min(2)}", Name = "GetStudentDBByID")]
        ...
        //opt1
        //GET   api/...?email=avio@fm,ai.com&pass=avi123  
        //public IHttpActionResult LoginStudent(string email, string pass)

        //opt2
        //GET   api/.../    {} 
        //public IHttpActionResult LoginStudent([FromBody] Student)

        [HttpPost]
        [Route("~/login")]
        public IHttpActionResult LoginStudent([FromBody] Student student2Login)
        {
            try
            {
                Student s2Login = StudentsDb.LoginStudent(student2Login.Email, student2Login.Password);
                if (s2Login != null)
                {
                    return Ok(s2Login);
                }
                else
                {
                    return Content(HttpStatusCode.NotFound, 
                        $" student with email={student2Login.Email}" +
                        $" and password={student2Login.Password} was not found for Login!");
                }
            }
            catch (Exception ex)
            {
        ...
        public IHttpActionResult Post([FromBody] Student value2Insert)
        {
            try
            {
                ...
                return Created(new Uri(Url.Link("GetStudentDBByID", new { id = value2Insert.ID } )) ,value2Insert);
            }
            catch (Exception ex)
            {
                return Content(HttpStatusCode.InternalServerError, ex);
